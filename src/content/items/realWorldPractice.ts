  ...
  (rng) => {
    ...
    return {
      ...
      answer: mcq(rng, winner, [
        centsA < centsB ? `Pack B, the larger ${sizeB}-unit pack` : `Pack A at ${money(priceA)} on the sticker`,
        'Equal — same product',
        'The larger pack, always',
      ]),
...
const meetingParaphrase = tpl(
  ...
  (rng) => {
    const msg = pick(rng, [
      { text: 'I can finish the slides tonight, but I need the final numbers by six.', para: 'You will be able to finish tonight if the final numbers arrive by 6 pm.' },
  ...
  (rng, seed) => {
    ...
    return {
      ...
      answer: mcq(rng, `“No, I’m not going to ${ask}. Please stop asking. I can help with a safe alternative if there is one.”`, [
        `“I really can’t right now, maybe another time — sorry, it’s all a bit complicated to explain properly.”`,
        `“I’m sorry, I feel really bad about this, I know you need it and I hate saying no, so please don’t be upset.”`,
        `“Stop asking me things like this. I’m completely done with this conversation, and with everyone involved in it.”`,
      ]),
  ...
  (rng, seed) => {
    ...
    return {
      ...
      answer: mcq(rng, 'Do not amplify the claim; check an official/primary channel, alert a responsible adult or staff member privately, and follow verified safety instructions', [
        'Pass it on with a clear note that it is unconfirmed, so that people nearby can judge the risk and decide for themselves what to do about it',
        'Treat it as false and say so publicly, since a real warning would have carried a time, a source, and an official notice',
        'Go and look for yourself first before saying anything, so that whatever you pass on afterwards is something you have verified',
      ]),
  ...
  (rng) => {
    ...
    return {
      ...
      answer: mcq(rng, 'Freeze optional polish, assign one owner to verify the numbers, define a minimum complete version, document attempts to reach the teammate, and tell the teacher/client early if scope or attribution must change', [
        'Write the missing section yourself overnight, keep the absent teammate credited as its author, move the two unverified numbers into an appendix where they draw less attention, and raise none of it unless the teacher asks first',
        'Put every remaining hour into presentation and layout, rehearse a confident delivery, describe the missing section as deliberately out of scope, and let the two unverified numbers stand, since nobody is likely to recheck them',
        'Hold off on contacting the teacher or the teammate until the deadline has passed, keep working down the original task list in order, and prepare a full explanation afterwards, since early warnings only invite extra scrutiny',
      ]),
