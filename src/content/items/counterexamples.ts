...
export const FALSE_CLAIMS: FalseClaim[] = [
  {
    ...
    supporters: [
      { label: 'x = 3 — squaring gives 9, which is larger', values: [3] },
      { label: 'x = 10 — squaring gives 100, which is larger', values: [10] },
      { label: 'x = 7 — squaring gives 49, which is larger', values: [7] },
    ],
  ...
  },
  {
    ...
    supporters: [
      { label: '3 + 5 = 8, which is even', values: [3, 5] },
      { label: '7 + 11 = 18, which is even', values: [7, 11] },
      { label: '13 + 17 = 30, which is even', values: [13, 17] },
    ],
  ...
  },
  {
    ...
    supporters: [
      { label: '16 — divisible by 2 and by 4, and 8 divides it too', values: [16] },
      { label: '24 — divisible by 2 and by 4, and 8 divides it too', values: [24] },
      { label: '40 — divisible by 2 and by 4, and 8 divides it too', values: [40] },
    ],
  ...
  },
  {
    ...
    supporters: [
      { label: 'A car accelerating away and its speedometer needle rising along with it', values: [2] },
      { label: 'A tap left running and the bucket underneath it slowly filling', values: [3] },
      { label: 'A switch being flipped and the lamp above it lighting up straight away', values: [4] },
    ],
    ...
    const c = cycle(seed, [
      {
        ...
        bads: [
          ['Squaring makes a number bigger whenever that number happens to be positive', 'fails at x = 1/2, which is positive and shrinks'],
          ['Squaring a number always makes it bigger, or else leaves it equal', 'still false: 1/4 is strictly smaller than 1/2'],
          ['Squaring a number never changes its size at all', 'far too strong — it is false for almost every number'],
      ...
      },
      {
        ...
        bads: [
          ['If a number happens to be divisible by both 2 and 4, then it is divisible by 6', 'still false — 4 itself passes both and fails 6'],
          ['Only even numbers can ever be divisible by 4', 'true but unrelated — it does not repair the broken implication'],
          ['Divisibility rules of this kind only ever work for prime divisors', 'too strong: divisibility by 6 and 12 works fine'],
        ] as [string, string, ErrorTag?][],
      },
      {
        ...
        bads: [
          ['The average of two class averages always comes out too high, in every case', 'it can land either side depending on which class is bigger'],
          ['You should always be using the median here instead of the mean', 'changes the tool rather than repairing the claim'],
          ['Averages of averages work perfectly well whenever the two averages are close together', 'closeness reduces the error but never makes the rule exact'],
        ] as [string, string, ErrorTag?][],
      },
    ] as const)
